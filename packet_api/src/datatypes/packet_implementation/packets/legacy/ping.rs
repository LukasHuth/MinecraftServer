...
use crate::{datatypes::packet_implementation::{bytes_to_short, packets::Packet}, DatastructError};
...
impl Packet for LegacyPing {
    fn read(stream: &mut BufReader<&mut TcpStream>) -> Result<LegacyPing, DatastructError> where Self: Sized {
    ...
    fn read_length(_stream: &mut BufReader<&mut TcpStream>, _length: crate::datatypes::datastructs::VarInt) -> Result<LegacyPing, DatastructError> where Self: Sized {
        unreachable!()
    }
}
