...
pub enum ClientboundPackets {
    ...
    LegacyPong(handshake::LegacyPongPacket),
    ...
    Status(status::StatusResponsePacket),
    ...
    AddResourcePack(configuration::clientbound::AddResourcePack),
    ...
    FeatureFlags(configuration::clientbound::FeatureFlags),
    /// Packet to tell the client that the configuration is finished and should switch the state to
    /// `Playing`
    FinishConfiguration(configuration::clientbound::FinishConfiguration),
}
...
pub enum ServerboundPackets {
    ...
    LoginEncryptionResponse(login::LoginEncryptionResponse),
    ...
    ClientInformation(configuration::serverbound::ClientInformation),
    /// Plugin message packet for the configuration phase
    ConfigurationPluginMessage(configuration::serverbound::ServerboundPluginMessage),
    ...
    KeepAlive(configuration::serverbound::KeepAliveResponse),
    ...
    ResoucePackResponse(configuration::serverbound::ResoucePackResponse),
}
impl ServerboundPackets {
        ...
        Self: 'a
    {
        ...
        if first_two_bytes[0] == 0xFE && first_two_bytes[1] == 0x01 {
            use handshake::LegacyPingPacket;
